...
impl Shader {
    unsafe fn load() -> anyhow::Result<Shader> {
        ...
        let vert_source = std::ffi::CString::from_vec_unchecked(include_bytes!("shader.vert").to_vec());
        let frag_source = std::ffi::CString::from_vec_unchecked(include_bytes!("shader.frag").to_vec());
...
impl Renderer {
    ...
    fn convert_paint(
        ...
        if scissor.extent.width < -0.5 || scissor.extent.height < -0.5 {
            ...
            frag.scissor_scale[0] =
                (scissor.xform.0[0] * scissor.xform.0[0] + scissor.xform.0[2] * scissor.xform.0[2]).sqrt() / fringe;
            frag.scissor_scale[1] =
                (scissor.xform.0[1] * scissor.xform.0[1] + scissor.xform.0[3] * scissor.xform.0[3]).sqrt() / fringe;
        }
    ...
    fn append_uniforms(&mut self, uniforms: FragUniforms) {
        self.uniforms.resize(self.uniforms.len() + self.frag_size, 0);
        unsafe {
...
impl nvg::renderer::Renderer for Renderer {
    ...
    fn flush(&mut self) -> anyhow::Result<()> {
        if !self.calls.is_empty() {
            unsafe {
                ...
                gl::Uniform2fv(self.shader.loc_viewsize, 1, &self.view as *const Extent as *const f32);
    ...
    fn fill(
        ...
        if call.call_type == CallType::Fill {
            ...
            self.vertexes.push(Vertex::new(bounds.max.x, bounds.max.y, 0.5, 1.0));
            self.vertexes.push(Vertex::new(bounds.max.x, bounds.min.y, 0.5, 1.0));
            self.vertexes.push(Vertex::new(bounds.min.x, bounds.max.y, 0.5, 1.0));
            self.vertexes.push(Vertex::new(bounds.min.x, bounds.min.y, 0.5, 1.0));
    ...
    fn stroke(
        ...
        self.append_uniforms(self.convert_paint(paint, scissor, stroke_width, fringe, 1.0 - 0.5 / 255.0));
...
fn shader_error(shader: gl::types::GLuint, filename: &str) -> anyhow::Error {
    unsafe {
        ...
        anyhow!("failed to compile shader: {}: {}", filename, err_msg.to_string_lossy())
    }
}
