...
pub trait Demo<R: Renderer> {
    fn init(&mut self, ctx: &mut Context<R>) -> anyhow::Result<()> {
        ctx.create_font_from_file("roboto", "./examples/Roboto-Bold.ttf")?;
        Ok(())
    }
...
pub fn run<D: Demo<nvg_gl_backend::Renderer> + 'static>(mut demo: D, title: &str) {
    ...
    el.run(move |evt, _, ctrl_flow| {
        ...
        match evt {
            ...
            Event::WindowEvent { event, .. } => match event {
                ...
                WindowEvent::CursorMoved { position, .. } => demo.cursor_moved(position.x as f32, position.y as f32),
                _ => (),
            },
            Event::RedrawRequested(_) => {
                ...
                context.save();
                demo.update(window_size.width as f32, window_size.height as f32, &mut context)
                    .unwrap();
                context.restore();
